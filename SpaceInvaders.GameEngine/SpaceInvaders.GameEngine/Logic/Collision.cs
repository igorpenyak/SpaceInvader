...
namespace SpaceInvaders.GameEngine
{
    public class Collision
    {
            ...
            for (var i = 0; i < game.i_arr.GetLength(0); i++)
            {
                for (var j = game.i_arr.GetLength(1) - 1; j >= 0; j--)
                {
                    if (game.i_arr[i, j].Live)
                    {
                        ...
                        //    }
                        //  }
                        CollisionLazerGun(game, i, j);
                        CollisionInvader(game, i, j);
        ...
        public static void CollisionInvader(Process game, int i, int j)
        {
        ...
        public static void CollisionLazerGun(Process game, int i, int j)
        {
            for (int b = 0; b < game.b_list.Count; b++)
            {
